...
class NoteApi extends Controller
{
    ...
    {
        $note = Note::where('user_id', $this->user($request)->id)->latest();
    ...
    {
        $note = Note::where('slug', $id)->where('user_id', $this->user($request)->id)
            ->with('category', 'user')
            ->first();
        if (!$note) {
    ...
    public function store(Request $request)
    {
        ...
        //check category
        $category = Category::where('slug', $request->category_slug)->where('user_id', $this->user($request)->id);
        if (!$category->first()) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $note = Note::where('user_id', $this->user($request)->id)->where('slug', $id);
        if (!$note->first()) {
